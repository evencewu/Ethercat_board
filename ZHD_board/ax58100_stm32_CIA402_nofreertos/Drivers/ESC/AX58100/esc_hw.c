...
static void esc_address(uint16_t address, uint8_t command)
{
    ...
    HAL_SPI_TransmitReceive(&hspi2, data, (uint8_t *)&ESCvar.ALevent, sizeof(data), 100);
...
void ESC_read(uint16_t address, void *buf, uint16_t len)
{
     ...
     */
    HAL_SPI_TransmitReceive(&hspi2, read_termination + (MAX_READ_SIZE - len), buf, len, 1000);
