      ...
      if (nsub >= 0)
      {
         ...
         if (WRITE_ACCESS(access, state))
         {
            ...
            if (coesdo->command & COE_EXPEDITED_INDICATOR)
            {
               ...
               uint32_t size_value = coesdo->size;
               mbxdata = &size_value;
            }
            else
            {
...
static void SDO_downloadsegment (void)
{
   ...
   if (MBXout)
   {
      ...
      uint32_t size_value = coesdo->index;
      uint32_t *mbxdata = &size_value;  /* data pointer */
...
static void SDO_getodlist (void)
{
   ...
   if (MBXout)
   {
      ...
      if (etohs (coer->index) == 0x00)
      {
         ...
         uint16_t size_value = coel->datatype;
         p = &size_value;
         *p = htoes (entries);
         p++;
         *p = 0;
...
static void SDO_getodlistcont (void)
{
   ...
   if (MBXout)
   {
      ...
      uint16_t size_value = coel->index;
      p = &size_value;
      for (i = s; i < n; i++)
      {
         *p = htoes (SDOobjects[i].index);
